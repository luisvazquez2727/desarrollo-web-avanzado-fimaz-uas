...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($nombre === "" || $apellido === "" || $correo === "") {
        ...
        try {
            ...
            $stmtLog = $pdo->prepare(
                "INSERT INTO logs_alumnos (idAlumno, accion)
            ...
            $pdo->commit();
            $mensaje = "✅ COMMIT: Alumno registrado con ID $idAlumno";

        } catch (Exception $e) {
            if ($pdo->inTransaction()) {
                $pdo->rollBack();
            }
            $mensaje = "❌ ROLLBACK: Error en la transacción";
...
<head>
    ...
    <style>
        ...
        .btn{background:#0056b7; color:white;}
        .btn:hover{opacity:.9}
    ...
    <form method="POST">
        <div class="row">
            <div>
                ...
                value="<?= htmlspecialchars($_POST['nombre'] ?? 'luis') ?>">
            </div>
            <div>
                <label>Apellido</label>
                <input type="text" name="apellido" maxlength="10"
                value="<?= htmlspecialchars($_POST['apellido'] ?? 'vazquez') ?>">
            </div>
            <div>
                <label>Correo</label>
                <input type="email" name="correo" maxlength="50"
                value="<?= htmlspecialchars($_POST['correo'] ?? 'lvgluisjesus@gmail.com') ?>">
            </div>
        </div>
